...
public class SaleBusiness : Business<Sale, SaleView, SaleUpdate, SaleModify, SaleCreate, SaleQuery>
{
    ...
    public override QueryResult<ClientQuery, SaleView> GetAll(SaleQuery clientQuery, DataQuery query, int maxDepth = 2)
    {
        ...
        var data = (sortedQ ?? q)
            .Select(x => new SaleView { Id = x.Id, CompanyId = x.CompanyId, AccountId = x.AccountId, CustomerId = x.CustomerId, CurrencyId = x.CurrencyId, Place = x.Place, Number = x.Number, Date = x.Date, Total = x.Total, TotalItems = x.TotalItems, Reference = x.Reference, Confirmed = x.Confirmed, ReferenceDate = x.ReferenceDate, DueDate = x.DueDate })
            .ToList();
